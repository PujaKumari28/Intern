...
route.get("/foods/:page", async (req, res) => {
    ...
    const foodsWithBase64 = foods.map(food => {
        return {
            ...food._doc,
            photo: food.photo ? food.photo.toString('base64') : '',
        };
    });

    console.log(count + " :=> " + foodsWithBase64);
    res.render("showDishes", {
        loginUser: loginUser,
        foods: foodsWithBase64,
        count: count,
...
route.post('/saveDish', async (req, res) => {
    try {
        if (!req.files || req.body.ddiscount > 100 || !req.body.dname || req.body.dprice <= 0) {
            return res.render("addNewDish", {
                notsave: true
            });
        }
        ...
        const newDish = new dish({
            ...
            ddiscount: req.body.ddiscount,
            dserve: req.body.dserve
        });
...
route.post("/orderNowFromBasket", (req, res) => {
    if (req.session.loginUser) {
        ...
        basket.forEach(async function (item) {
            const base64Data = item.image.replace(/^data:image\/jpeg;base64,/, ""); // Remove the base64 prefix
            ...
            let object = {
                ...
                price: item.price,
                photo: imageBuffer,
                photoType: "image/jpeg", // Assuming the image type is JPEG
                time: dateTime,
...
route.get("/user/history", async (req, res) => {
    if (req.session.loginUser) {
        ...
        const history = data.map(item => ({
            ...item._doc,
            photo: item.photo ? item.photo.toString('base64') : '',
        }));
